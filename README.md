...
- Import CLI-generated summaries directly into the UI and automatically group queued samples by sample name, marker, or a custom cohort.
...
### CLI summary imports
- Load summary tables generated by the CLI runner to reproduce results inside the Streamlit editor.
- When re-queuing samples, use **Post-import grouping** to automatically assign stems to alignment groups by sample column, marker name, or a custom group label.
- Marker-based grouping ensures multi-marker uploads are split into separate groups so each marker can be aligned independently.
