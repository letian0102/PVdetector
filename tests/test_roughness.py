...
def test_roughness_failure_falls_back_to_scott(monkeypatch):
    rng = np.random.default_rng(321)
    counts = rng.normal(0, 1, 200)

    def explode(_counts):  # pragma: no cover - exercised via test
        raise RuntimeError("boom")

    monkeypatch.setattr("peak_valley.batch.find_bw_for_roughness", explode)

    options = BatchOptions(bandwidth="roughness")
    params, debug = _resolve_parameters(
        options,
        {"bw": "roughness", "n_peaks": "auto"},
        None,
    ...
    assert params["bandwidth_effective"] == "scott"
    assert debug.get("bandwidth_method") == "roughness_fallback"
    assert "roughness_error" in debug
